...
@section('header')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl sm:text-3xl font-extrabold text-gray-900 tracking-tight">Blog y Tutoriales</h1>
            <p class="text-sm text-gray-500 mt-1">Gestiona los artículos y tutoriales SEO de la tienda.</p>
        </div>
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <div class="text-xs text-gray-500 mt-0.5">{{ $post->created_at->format('d M, Y') }} por {{ $post->author->name ?? 'Admin' }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('blog.show', $post->slug) }}" target="_blank" class="w-8 h-8 inline-flex items-center justify-center rounded-lg text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-colors" title="Ver Artículo">
                                    <i class="fa-solid fa-eye"></i>
                                </a>
                                <a href="{{ route('admin.blog.edit', $post) }}" class="w-8 h-8 inline-flex items-center justify-center rounded-lg text-gray-400 hover:text-amber-600 hover:bg-amber-50 transition-colors" title="Editar">
                                ...
                                <form action="{{ route('admin.blog.destroy', $post) }}" method="POST" class="inline" onsubmit="return confirm('¿Estás seguro de querer eliminar este artículo?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-8 h-8 inline-flex items-center justify-center rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors" title="Eliminar">
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-12 text-center text-gray-500">
                            <div class="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-3">
                                <i class="fa-solid fa-pen-nib text-2xl text-gray-400"></i>
                            </div>
                            <p class="font-medium text-gray-900 mb-1">Aún no hay artículos</p>
