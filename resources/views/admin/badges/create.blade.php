...
<span class="text-gray-900 dark:text-white font-medium">Crear</span>
@endsection

@section('content')
<form action="{{ route('admin.badges.store') }}" method="POST" class="max-w-2xl mx-auto">
    @csrf
    ...
    <div class="bg-white dark:bg-[#111827] rounded-2xl border border-gray-200/60 dark:border-gray-800/60 p-6 space-y-6">
        <div>
            ...
            <input type="text" name="name" value="{{ old('name') }}" required maxlength="20" class="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 rounded-xl text-[13px] text-gray-900 dark:text-white placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-400/30 focus:border-primary-400 transition-all" placeholder="Ej: HOT, OFERTA, NUEVO">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <select name="color" class="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 rounded-xl text-[13px] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-400/30 focus:border-primary-400 transition-all">
                    ...
                    <option value="purple" {{ old('color') == 'purple' ? 'selected' : '' }}>Morado</option>
            ...
            <div>
                ...
                <select name="icon" class="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 rounded-xl text-[13px] text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-400/30 focus:border-primary-400 transition-all font-mono">
                    ...
                    <option value="fa-solid fa-fire" {{ old('icon') == 'fa-solid fa-fire' ? 'selected' : '' }}>🔥 fa-fire</option>
                    <option value="fa-solid fa-star" {{ old('icon') == 'fa-solid fa-star' ? 'selected' : '' }}>⭐ fa-star</option>
                    <option value="fa-solid fa-tag" {{ old('icon') == 'fa-solid fa-tag' ? 'selected' : '' }}>🏷️ fa-tag</option>
                    <option value="fa-solid fa-bolt" {{ old('icon') == 'fa-solid fa-bolt' ? 'selected' : '' }}>⚡ fa-bolt</option>
                    <option value="fa-solid fa-sparkles" {{ old('icon') == 'fa-solid fa-sparkles' ? 'selected' : '' }}>✨ fa-sparkles</option>
                    <option value="fa-solid fa-crown" {{ old('icon') == 'fa-solid fa-crown' ? 'selected' : '' }}>👑 fa-crown</option>
                    <option value="fa-solid fa-clock" {{ old('icon') == 'fa-solid fa-clock' ? 'selected' : '' }}>🕒 fa-clock</option>
                </select>
