                ...
                <div class="grid grid-cols-2 gap-y-6 gap-x-4">
                    <div class="flex gap-3">
                        <div class="w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center shrink-0 border border-gray-100">
                            ...
                            @if(count($platformList) > 1)
                                {{-- Múltiples plataformas: mostrar iconos en fila --}}
                                <div class="flex items-center gap-1 flex-wrap">
                                    @foreach($platformList as $pl)
                                        <span title="{{ $pl }}">{!! $getPlatformIcon($pl) !!}</span>
                                    @endforeach
                                </div>
                            @else
                                {!! $getPlatformIcon($platformsRaw) !!}
                            @endif
                        </div>
                        <div>
                        <div class="text-[11px] text-gray-500 font-semibold uppercase tracking-wider">Plataforma</div>
                            <div class="text-sm font-bold text-gray-900">
                                {{ count($platformList) > 1 ? implode(', ', $platformList) : ($product->platform ?? 'PC') }}
                            </div>
                        </div>
                    </div>
